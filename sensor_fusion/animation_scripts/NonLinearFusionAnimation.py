...
def test_full_fusion():
    ...
    filter_list = [ekf_no_sensor, ekf_gps_only, ekf_mag_only, ekf_full_fusion]
    ...
    for t in range(simulation_time * f_control):
        ...
        for estimator in filter_list:
            estimator.control_update(controls[t])
    ...
    for estimator in filter_list:
        print(f"Mean Error ({estimator}): {np.mean(np.abs(estimator.get_estimated_states() - car_states)):.3f}")

    make_animation(filter_list, car)


def make_animation(filter_list, car):
    ...
    a3.set_xlim(0, 70)
    a3.set_ylim(0, 2)
    for a in (a1, a2, a3): a.grid(True)
    a1.set_title("Vehicle position")
    a2.set_ylabel("Position \nCovariance")
    a3.set_ylabel("Heading \nCovariance")
    a3.set_xlabel("Time")
    a3.set_xticklabels([i/10 for i in a3.get_xticks()])
